...
RC PhysicalPlanGenerator::create(LogicalOperator &logical_operator, unique_ptr<PhysicalOperator> &oper)
{
  ...
  switch (logical_operator.type()) {
    ...
    case LogicalOperatorType::UPDATE: { 
      /*为了新增Update操作的物理实现 在此增加此分支 否则会在下面走到assert
       *接下来要实现Update操作专属的create_plan函数
       */
...
/*为该文件头顶的PhysicalPlanGenerator::create函数中的switch模块而写的Update的物理计划生成函数
 *已在PhysicalPlanGenerator中声明
 *本文件将需求UpdatePhysicaloperator类的实现
 */
{
